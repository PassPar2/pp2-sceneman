...
local menu2 = MenuV:CreateMenu(false, Lang:t("sceneman.ped_density_options"), sceneManagerConfig.menuLocation, 220, 20, 60, 'size-125', 'none', 'menuv', 'test2')
...
vehiculesDensityValue:On("select", function(_, value)
  TriggerServerEvent("pp2-sceneman:server:changeGlobalDensity", value/10, 'vehicle', 'vehicules')
end)
...
parkedVehiculesDensityValue:On("select", function(_, value)
  TriggerServerEvent("pp2-sceneman:server:changeGlobalDensity", value/10, 'parked', 'parked vehicules')
end)
...
pedDensityValue:On("select", function(_, value)
  TriggerServerEvent("pp2-sceneman:server:changeGlobalDensity", value/10, 'peds', 'peds')
end)
...
scenarioPedDensityValue:On("select", function(_, value)
  TriggerServerEvent("pp2-sceneman:server:changeGlobalDensity", value/10, 'scenario', 'scenario')
end)
...
multiplierDensityValue:On("select", function(_, value)
  TriggerServerEvent("pp2-sceneman:server:changeGlobalDensity", value/10, 'multiplier', 'multiplier')
end)

RegisterNetEvent("p2-sceneman:client:changeDensity", function(newDensity, newDensityType, newDensityTypeLabel)
  changeTypeDensity(newDensity, newDensityType, newDensityTypeLabel)
end)
